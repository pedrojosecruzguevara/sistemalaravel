...
class PublicacionesController extends Controller
{
  ...
  {
    //función para agregar la publicación de cada usuario
    $archivo = $request->file('file');
    $input  = array('file' => $archivo);
    $reglas = array('file' => 'required|mimes:pdf|max:50000');  //recordar que para activar mimes se debe descomentar la línea de código  'extension=php_fileinfo.dll' del php.ini
    ...
    if ($validacion->fails()) {
      ...
      if ($resul) {
        return view("mensajes.msj_correcto")->with("msj", "Publicación Agregada Correctamente");
      } else {
